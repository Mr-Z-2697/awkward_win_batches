...
for %%x in (%*) do (
    ffmpeg -hide_banner -r 1 -i "%%~dpnxx" -vf scale=out_color_matrix=470bg:out_range=pc:flags=lanczos:sws_dither=ed,format=yuv444p10le -frames 1 -c:v libx265 -preset 6 -crf %q% -x265-params no-sao=1:selective-sao=0:ref=1:bframes=0:aq-mode=1:psy-rd=2:psy-rdoq=8:cbqpoffs=1:crqpoffs=1:range=full:colormatrix=bt470bg:transfer=iec61966-2-1:no-info=1 "%temp%\%%~nxx.heic.hevc" -y
    ffmpeg -hide_banner -r 1 -i "%%~dpnxx" -vf extractplanes=a,scale=out_range=pc,format=yuv444p10le -frames 1 -c:v libx265 -preset 6 -crf %q% -x265-params no-sao=1:selective-sao=0:ref=1:bframes=0:aq-mode=1:psy-rd=2:psy-rdoq=8:cbqpoffs=1:crqpoffs=1:range=full:colormatrix=bt470bg:transfer=iec61966-2-1:no-info=1 "%temp%\%%~nxx.alpha.hevc" -y
